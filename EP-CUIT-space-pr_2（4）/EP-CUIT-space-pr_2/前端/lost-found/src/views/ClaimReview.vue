...
async function rejectClaim(claimId) {
  ...
  try {
    ...
    if (data.success) {
      ...
      alert("驳回失败: " + (data.message || "请重试"));
    }
  } catch (error) {
    console.error("驳回认领申请失败:", error);
    alert("驳回失败，请检查网络连接后重试");
  }
}
...
认领人信息：
- 用户名：${claim.username || "未填写"}
- 姓名：${claim.applicant_name || "未填写"}
- 联系方式：${claim.applicant_phone || "未填写"}
