...
async function updateItemStatus(item, status) {
  ...
  if (status === 1) {
    confirmMessage = item.type === 0 ? '确定要标记为"已找到"吗？' : '确定要标记为"已归还"吗？';
    successMessage = item.type === 0 ? '标记为"已找到"成功！' : '标记为"已归还"成功！';
  } else {
    confirmMessage = item.type === 0 ? '确定要取消"已找到"吗？物品状态将变为"未找到"。' : '确定要取消"已归还"吗？物品状态将变为"未被认领"。';
    successMessage = item.type === 0 ? '已取消"已找到"，状态变为"未找到"' : '已取消"已归还"，状态变为"未被认领"';
  }
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="item in filteredPublishList" :key="item.item_id">
                  ...
                  <td>
                    <a href="#" @click.stop="viewDetail(item)" class="action-btn view-btn">查看</a>
                    <a v-if="item.status === 0 && item.type === 0" href="#" @click.stop="updateItemStatus(item, 1)" class="action-btn status-btn">已找到</a>
                    <a v-if="item.status === 0 && item.type === 1" href="#" @click.stop="updateItemStatus(item, 1)" class="action-btn status-btn">已归还</a>
                    <a v-if="item.status === 1 && item.type === 0" href="#" @click.stop="updateItemStatus(item, 0)" class="action-btn status-btn" style="background: #f59e0b;">取消找到</a>
                    <a v-if="item.status === 1 && item.type === 1" href="#" @click.stop="updateItemStatus(item, 0)" class="action-btn status-btn" style="background: #f59e0b;">取消归还</a>
                    <a href="#" @click.stop="deleteItem(item.item_id)" class="action-btn delete-btn">删除</a>
                  </td>
                </tr>
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="item in claimList" :key="item.claim_id">
                  ...
                  <td>{{ item.create_time }}</td>
                  <td>{{ formatStatus(item.status, item.type) }}</td>
                  <td><a href="#" @click.stop="viewDetail(item)" class="action-btn view-btn">查看</a></td>
                </tr>
