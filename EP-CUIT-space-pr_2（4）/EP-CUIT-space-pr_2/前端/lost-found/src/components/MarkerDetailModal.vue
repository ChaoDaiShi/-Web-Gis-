...
function getClaimButtonText() {
  const itemStatus = props.marker?.status;
  return itemStatus === 0 ? '申请归还' : '申请认领';
}

function handleClaim() {
  if (!canClaim()) {
    alert('只能申请处理其他用户发布的物品');
    return;
  }
  
  const itemStatus = props.marker?.status;
  const itemId = props.marker?.id;
  
  if (itemStatus === 0) {
    // 丢失物品 - 申请归还
    window.location.href = itemId ? `/return-form?id=${itemId}` : '/return-form';
  } else {
    // 拾到物品 - 申请认领
    window.location.href = itemId ? `/claim-form?id=${itemId}` : '/claim-form';
  }
}
  ...
  <div class="marker-detail-modal-mask" :style="{ display: show ? 'flex' : 'none' }">
    <div class="marker-detail-modal">
      ...
      <div class="modal-body">
        ...
        <div v-if="!props.hidePublisher" class="publisher-section">
      ...
      <div class="modal-footer">
        <button 
          v-if="canClaim() && !props.disableActions" 
          class="btn claim" 
          @click="handleClaim"
        >{{ getClaimButtonText() }}</button>
        <button 
