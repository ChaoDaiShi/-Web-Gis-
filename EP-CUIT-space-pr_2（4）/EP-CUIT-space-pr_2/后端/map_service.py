...
def update_item_status(item_id):
    ...
    try:
        ...
        if not item:
            return jsonify({"success": False, "message": "物品不存在或无权限"}), 404
        ...
        if status == 1 and item_type == 0:
            cursor.execute("""
                INSERT INTO claim (item_id, claimer_id, message, status, create_time)
                VALUES (%s, %s, %s, %s, NOW())
            """, (item_id, user_id, "用户自行找回物品", 1))
        elif status == 0 and item_type == 0:
            cursor.execute("""
                DELETE FROM claim 
                WHERE item_id = %s AND claimer_id = %s AND message = %s
            """, (item_id, user_id, "用户自行找回物品"))
