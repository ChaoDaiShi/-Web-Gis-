...
class ReturnForm(db.Model):
    __tablename__ = 'return_form'
...
def get_return_forms():
    return_forms = ReturnForm.query.all()
    result = []
...
def add_return_form():
    try:
        ...
        if not return_reason or not item_description:
            return jsonify({'success': False, 'message': '归还理由和物品描述不能为空'})
        ...
        return jsonify({'success': True, 'message': '提交成功', 'return_id': new_form.return_id})
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': str(e)})
...
def send_claim_notification(user_id, is_approved, item_id=None, item_title=None):
    """发送认领申请审核结果通知"""
    if not user_id:
        return
...
def approve_claim_form(claim_id):
    ...
    try:
        ...
        send_claim_notification(claim_form.user_id, True, claim_form.item_id)
...
def reject_claim_form(claim_id):
    ...
    try:
        user_id = claim_form.user_id
        item_id = claim_form.item_id
...
def send_return_notification(user_id, is_approved, item_id=None, item_title=None):
    """发送归还申请审核结果通知"""
    if not user_id:
        return
...
@admin_api_bp.route('/return-forms/<int:return_id>/approve', methods=['POST'])
def approve_return_form(return_id):
    return_form = ReturnForm.query.get(return_id)
    if not return_form:
        return jsonify({'success': False, 'message': '归还记录不存在'})
    
    try:
        return_form.status = 1
        db.session.commit()
        
        send_return_notification(return_form.user_id, True, return_form.item_id)
...
@admin_api_bp.route('/return-forms/<int:return_id>/reject', methods=['POST'])
def reject_return_form(return_id):
    ...
    try:
        user_id = return_form.user_id
        item_id = return_form.item_id
