...
def audit_item(item_id):
    ...
    try:
        ...
        # 更新 lost_item 的 audit_status（仅保留此冗余字段用于高效查询过滤）
        updates = ["audit_status = %s"]
        params = [audit_status]
        ...
        cursor.execute("""
            UPDATE audit 
            SET status = %s, audit_time = %s, audit_remark = %s, update_time = %s
            WHERE target_type = 'lost_item' AND target_id = %s
        """, (audit_status, now, audit_remark, now, item_id))
