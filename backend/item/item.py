...
from models import LostItem, ItemCategory, ClaimRecord, SystemLog
...
def claim_item(item_id):
    ...
    existing_claim = ClaimRecord.query.filter_by(item_id=int(item_id), claimer_id=int(current_user_id)).first()
    ...
    claim = ClaimRecord(
        item_id=int(item_id),
        claimer_id=int(current_user_id),
        claim_reason=data.get('claim_reason'),
        proof_info=data.get('proof_info'),
        status='pending'
    )
