...
from common.db_config import get_conn as get_db_connection
...
def get_central_items():
    ...
    try:
        ...
        for item in items:
            if item['image_urls']:
                image_keys = json.loads(item['image_urls'])
                item['image_urls'] = [get_signed_url(key) for key in image_keys if key]
            else:
                item['image_urls'] = []
...
def get_my_appointments():
    ...
    try:
        ...
        for app in appointments:
            if app['image_urls']:
                image_keys = json.loads(app['image_urls'])
                app['image_urls'] = [get_signed_url(key) for key in image_keys if key]
            else:
                app['image_urls'] = []
