...
function handleAutofill() {
  nextTick(() => {
    // 尝试读取DOM中的值并同步到ref
    const loginInputEl = document.querySelector('input[name="loginInput"]');
    const passwordEl = document.querySelector('input[name="password"]');
    const adminUsernameEl = document.querySelector('input[name="adminUsername"]');
    const adminPasswordEl = document.querySelector('input[name="adminPassword"]');
    
    if (loginInputEl && loginInputEl.value) {
      loginInput.value = loginInputEl.value;
    }
    if (passwordEl && passwordEl.value) {
      password.value = passwordEl.value;
    }
    if (adminUsernameEl && adminUsernameEl.value) {
      adminUsername.value = adminUsernameEl.value;
    }
    if (adminPasswordEl && adminPasswordEl.value) {
      adminPassword.value = adminPasswordEl.value;
    }
  });
}
...
function updateAdminUsername(e) {
  adminUsername.value = e.target.value;
}

function updateAdminPassword(e) {
  adminPassword.value = e.target.value;
}
          ...
          <div v-show="mode === 'login'" class="form-group">
            ...
            <div class="input-wrapper">
              <input 
                ...
                @change="updateLoginInput"
                placeholder="邮箱或用户名（含@为邮箱）" 
              />
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input 
                ...
                name="adminUsername"
                autocomplete="username"
                @input="updateAdminUsername"
                @change="updateAdminUsername"
