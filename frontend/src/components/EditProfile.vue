...
async function loadUser() {
  try {
    ...
    if (data.success && data.data) {
      ...
      avatar.value = data.data.avatar ? (data.data.avatar.startsWith('http') ? data.data.avatar : `http://127.0.0.1:5000${data.data.avatar}`) : "";
      
      if (data.data.bg_image) {
        bgImage.value = data.data.bg_image.startsWith('http') ? data.data.bg_image : `http://127.0.0.1:5000${data.data.bg_image}`;
      }
    }
  } catch (error) {
