...
import { createFilePreview, revokeFilePreview } from '../utils/uploadHelper.js';
...
watch(() => props.show, (newVal) => {
  if (newVal) {
    ...
    pubType.value = 'lost';
    pubImages.value = [];
    pubImagePreviews.value = [];
    localPendingLocation.value = props.pendingLocation;
    loadCategories();
  } else {
    pubImagePreviews.value.forEach(preview => revokeFilePreview(preview));
    pubImagePreviews.value = [];
  }
});
...
function handleImageUpload(event) {
  ...
  const newPreviews = [...pubImagePreviews.value];
  ...
  pubImagePreviews.value = newPreviews;
...
function removeImage(index) {
  revokeFilePreview(pubImagePreviews.value[index]);
  pubImages.value.splice(index, 1);
  pubImagePreviews.value.splice(index, 1);
}

function getImageUrl(img, index) {
  if (pubImagePreviews.value[index]) {
    return pubImagePreviews.value[index];
  }
  ...
  try {
    if (typeof URL !== 'undefined' && URL.createObjectURL) {
      ...
      pubImagePreviews.value[index] = preview;
      return preview;
    }
  } catch (e) {
    ...
    <div class="publish-modal">
      ...
      <div class="modal-footer">
        ...
        <button class="btn primary" type="button" @click="handleSubmit" :disabled="isSubmitting">
          <span v-if="isSubmitting && isUploadingImages">上传图片中... {{ uploadProgress }}%</span>
          <span v-else-if="isSubmitting">发布中...</span>
          <span v-else>发布</span>
        </button>
      </div>
    </div>
