...
watch(() => props.categories, (newVal) => {
  if (newVal && newVal.length > 0 && pubCategory.value === 1) {
    const otherCategory = newVal.find(cat => cat.name === '其他');
    if (otherCategory) {
      pubCategory.value = otherCategory.id;
    } else {
      pubCategory.value = newVal[0].id;
    }
  }
}, { immediate: true });
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          ...
          <select id="pubCategory" v-model="pubCategory">
            <option v-if="categories.length === 0" value="">加载中...</option>
