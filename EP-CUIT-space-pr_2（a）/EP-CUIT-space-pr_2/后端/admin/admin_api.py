    ...
    try:
        ...
        if status == 'approved':
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s)
            """, (claim[0], claim[1], datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'approved'))
...
def create_category():
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO category (name, description, sort_order, is_active)
            VALUES (%s, %s, %s, %s)
...
def update_category(category_id):
    ...
    try:
        ...
        if 'name' in data:
            cursor.execute("SELECT * FROM category WHERE name = %s AND id != %s", (data['name'], category_id))
            existing = cursor.fetchone()
            if existing:
                return jsonify({'code': 400, 'message': '分类名称已存在'}), 400
            
            cursor.execute("UPDATE category SET name = %s WHERE id = %s", (data['name'], category_id))
        
        if 'description' in data:
            cursor.execute("UPDATE category SET description = %s WHERE id = %s", (data['description'], category_id))
        if 'sort_order' in data:
            cursor.execute("UPDATE category SET sort_order = %s WHERE id = %s", (data['sort_order'], category_id))
        if 'is_active' in data:
            cursor.execute("UPDATE category SET is_active = %s WHERE id = %s", (data['is_active'], category_id))
...
def add_claim_form():
    try:
        ...
        print(f"DEBUG: 表单数据:")
        print(f"DEBUG:   applicant_name: {applicant_name}")
        print(f"DEBUG:   applicant_phone: {applicant_phone}")
        print(f"DEBUG:   claim_reason: {claim_reason[:50]}..." if len(claim_reason) > 50 else f"DEBUG:   claim_reason: {claim_reason}")
        
        print(f"DEBUG: 请求文件列表: {list(request.files.keys())}")
        print(f"DEBUG: 文件数量: {len(request.files)}")
            ...
            for key, file in request.files.items():
                if key.startswith('proof_images') and file and file.filename:
                    ...
                    if ext in ['.jpg', '.jpeg', '.png', '.gif']:
                        ...
                        data_url = f"data:{mime_type};base64,{base64_encoded}"
                        proof_images.append(data_url)
                        print(f"DEBUG: 图片已转换为Base64并准备存入数据库")
                    else:
                        print(f"DEBUG: 文件格式不支持: {file.filename}")
        else:
            print(f"DEBUG: 没有收到任何文件")
        
        print(f"DEBUG: 最终图片列表: {proof_images}")
...
@admin_api_bp.route('/claim-forms/<int:claim_id>', methods=['DELETE'])
